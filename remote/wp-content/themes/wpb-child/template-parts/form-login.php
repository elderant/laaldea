...
<form name="<?php echo $args['form_id']?>" id="<?php echo $args['form_id']?>" action="<?php echo esc_url( site_url( 'wp-login.php', 'login_post' ) )?>" method="post">
    ...
    <div class="form-row login-username">
      ...
      <input placeholder="<?php echo mb_strtoupper(esc_html( $args['label_username'] ), "UTF-8");?>"type="text" name="log" id="<?php echo esc_attr( $args['id_username'] )?>" class="input user-flow-input" value="<?php echo esc_attr( $args['value_username'] )?>" size="20"/>
    </div>
    <div class="form-row login-password">
      ...
      <input placeholder="<?php echo mb_strtoupper(esc_html( $args['label_password'] ), "UTF-8");?>" type="password" name="pwd" id="<?php echo esc_attr( $args['id_password'] )?>" class="input user-flow-input" value="" size="20"/>
    </div>
    <?php echo $login_form_middle;?>
    <?php if($args['remember']) : ?>
      <div class="form-row login-remember">
        <label class="user-flow-label h5">
          <input name="rememberme" type="checkbox" id="<?php echo mb_strtoupper(esc_attr( $args['id_remember'] ), "UTF-8");?>" value="forever"<?php ( $args['value_remember'] ? ' checked="checked"' : '' )?>/>
